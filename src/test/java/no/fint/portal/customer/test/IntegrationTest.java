...
public class IntegrationTest {
  ...
  public void assets() throws Exception {
    ...
    mockMvc.perform(get("/api/adapters/testing/testadapter@adapter.test.no")).andExpect(status().isOk()).andExpect(jsonPath("$.name").value(equalTo("testAdapter@adapter.test.no")));
    mockMvc.perform(get("/api/adapters/testing/testadapter@adapter.test.no/secret")).andExpect(status().isOk()).andExpect(content().string(containsString("_ClientSecret")));
    mockMvc.perform(put("/api/adapters/testing/testadapter@adapter.test.no/password").content("This is the new password").contentType(MediaType.TEXT_PLAIN)).andExpect(status().isOk());
    mockMvc.perform(put("/api/adapters/testing/testadapter@adapter.test.no").content("{ \"name\": \"testadapter@adapter.test.no\", \"note\": \"Test Adapter With New Note\", \"shortDescription\": \"This is a Brand Spanking New Test Adapter\" }").contentType(MediaType.APPLICATION_JSON)).andExpect(status().isOk()).andExpect(jsonPath("$.shortDescription").value(containsString("Spanking")));
    mockMvc.perform(put("/api/assets/testing/test_test_no/adapters/testadapter@adapter.test.no")).andExpect(status().is(204));
    mockMvc.perform(put("/api/components/administrasjon_personal/testing/adapters/testadapter@adapter.test.no").contentType(MediaType.APPLICATION_JSON)).andExpect(status().is(204));
    mockMvc.perform(delete("/api/components/administrasjon_personal/testing/adapters/testadapter@adapter.test.no")).andExpect(status().is(204));
    mockMvc.perform(delete("/api/components/administrasjon_personal/testing/adapters/testadapter@adapter.test.no")).andExpect(status().is(204));
    mockMvc.perform(delete("/api/adapters/testing/testadapter@adapter.test.no")).andExpect(status().is(204));
    mockMvc.perform(get("/api/clients/testing")).andExpect(status().is(200));
    mockMvc.perform(post("/api/clients/testing").content("{ \"name\": \"testclient\", \"note\": \"Test Client\", \"secret\": \"password\", \"shortDescription\": \"This is a Test Client.\" }").contentType(MediaType.APPLICATION_JSON)).andExpect(status().is(201)).andExpect(jsonPath("$.name").value(equalTo("testclient@client.test.no")));
    mockMvc.perform(get("/api/clients/testing/testclient@client.test.no")).andExpect(status().isOk()).andExpect(jsonPath("$.name").value(equalTo("testclient@client.test.no")));
    mockMvc.perform(get("/api/clients/testing/testclient@client.test.no/secret")).andExpect(status().isOk()).andExpect(content().string(containsString("_ClientSecret")));
    mockMvc.perform(put("/api/clients/testing/testclient@client.test.no").content("{ \"name\": \"testclient@client.test.no\", \"note\": \"Testing Client\", \"shortDescription\": \"This is an updated Test Client.\" }").contentType(MediaType.APPLICATION_JSON)).andExpect(status().isOk()).andExpect(jsonPath("$.shortDescription").value(containsString("updated")));
    mockMvc.perform(put("/api/clients/testing/testclient@client.test.no/password").content("This is the new password").contentType(MediaType.TEXT_PLAIN)).andExpect(status().isOk());
    mockMvc.perform(put("/api/assets/testing/test_test_no/clients/testclient@client.test.no")).andExpect(status().is(204));
    mockMvc.perform(put("/api/components/administrasjon_personal/testing/clients/testclient@client.test.no").contentType(MediaType.APPLICATION_JSON)).andExpect(status().is(204));
    mockMvc.perform(delete("/api/assets/testing/test_test_no/clients/testclient@client.test.no")).andExpect(status().is(204));
    mockMvc.perform(delete("/api/components/administrasjon_personal/testing/clients/testclient@client.test.no")).andExpect(status().is(204));
    mockMvc.perform(delete("/api/clients/testing/testclient@client.test.no")).andExpect(status().is(204));
